...
- На странице задачи Pyrus рядом с action-кнопками появляется «Сетап Чистилки» (перед кнопкой «Чистилка»).
- Блок action-кнопок в Pyrus перегруппировывается по секциям «Программисты» и «Менеджеры» с фиксированным порядком кнопок.
- При нехватке ширины каждая секция автоматически переключается с двух колонок на одну, чтобы кнопки не вылезали за контейнер.
