...
(function (global) {
    function getXlsxLibraryMissingError(context) {
        ...
        return (
            "Для разбора файла OpenEnds (XLSX) не найдена библиотека XLSX. " +
            "Проверьте, что fallback-загрузка xlsx.full.min.js в content script завершилась успешно."
        );
    }

    function parseOpenEndsFromXlsxArrayBuffer(arrayBuffer, options) {
        const config = options && typeof options === "object" ? options : {};
        const serializable = Boolean(config.serializable);
        const logHeader = Boolean(config.logHeader);
        const contextLabel = config.contextLabel === "background" ? "background" : "page";
        ...
        if (typeof XLSX === "undefined" || typeof XLSX.read !== "function") {
            return {
                ok: false,
                error: getXlsxLibraryMissingError(contextLabel)
            };
        }
        ...
        try {
            ...
            const suffix = contextLabel === "background" ? " в background" : "";
            console.error(`[QGA] Ошибка XLSX.read при разборе OpenEnds${suffix}:`, error);
        ...
        try {
            ...
            const suffix = contextLabel === "background" ? " в background" : "";
            console.error(`[QGA] Ошибка XLSX.utils.sheet_to_json при разборе OpenEnds${suffix}:`, error);
    ...
    function parseRatingFromXlsxArrayBuffer(arrayBuffer, options) {
        const config = options && typeof options === "object" ? options : {};
        const contextLabel = config.contextLabel === "background" ? "background" : "page";
        ...
        try {
            ...
            const suffix = contextLabel === "background" ? " в background" : "";
            console.warn(`[QGA] Ошибка XLSX.read при разборе рейтинга${suffix}:`, error);
    ...
    global.parseOpenEndsFromXlsxArrayBuffer = parseOpenEndsFromXlsxArrayBuffer;
    global.parseRatingFromXlsxArrayBuffer = parseRatingFromXlsxArrayBuffer;
